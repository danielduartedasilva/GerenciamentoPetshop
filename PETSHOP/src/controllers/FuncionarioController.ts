...
class FuncionarioController{
    async cadastrar(request: Request, response: Response) {
        try {
            ...
            response.status(201).json({ data: funcionario, error: false, msg: "Funcionário cadastrado com sucesso!", });
        } catch (error) {
            response.status(400).json({ data: error, error: true, msg: "Não foi possível cadastrar o funcionário.", });
        }   
    }
